...
function makeQuestion(diff: Difficulty): Question {
  ...
  while (distractors.size < 3) {
    const delta = rand(
      -Math.max(2, Math.floor(answer / 3) + 1),
      Math.max(3, Math.floor(answer / 3) + 2),
    );
...
function useThreeScene(mountRef: React.RefObject<HTMLDivElement | null>) {
  ...
  useEffect(() => {
    ...
    const hair = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ color: 0x3a2e2a }),
    );
    ...
    const mouth = new THREE.Mesh(new THREE.TorusGeometry(0.09, 0.025, 8, 16, Math.PI), mouthMat);
...
export function MathAdventure({ onBack, onComplete }: Props) {
  ...
  const accuracy = useMemo(
    () =>
      questionNum === 0
        ? 0
        : Math.round((correct / Math.max(1, questionNum + (phase === "end" ? 1 : 0))) * 100),
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="rounded-3xl border-4 border-card bg-card/95 p-5 text-center shadow-lg backdrop-blur md:p-6">
                ...
                <p className="mt-2 text-sm font-bold text-muted-foreground">
                  Show <span className="text-primary">1, 2, 3, or 4</span> fingers to pick an
                  answer.
                </p>
              </div>
