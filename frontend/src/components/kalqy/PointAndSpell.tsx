...
// Offline fallback vocabulary — the live word bank comes from the backend
// (GET /api/words). Sessions ramp difficulty by word length.
...
function pickSessionWords(bank: WordItem[]): WordItem[] {
  return shuffle(bank)
    .slice(0, WORDS_PER_SESSION)
    .sort((a, b) => a.word.length - b.word.length);
}

function pickOptions(target: WordItem, bank: WordItem[]): WordItem[] {
  const decoys = shuffle(bank.filter((w) => w.word !== target.word)).slice(
    0,
    OPTIONS_PER_ROUND - 1,
  );
...
export function PointAndSpell({ onBack, onComplete }: Props) {
  ...
  // Word bank arrives from the backend on mount; empty until then.
  const bankRef = useRef<WordItem[]>(WORD_BANK);
  ...
  const startFindRound = useCallback((word: WordItem) => {
    setMode("find");
    setOptions(pickOptions(word, bankRef.current));
    setWrongPick(null);
    setCorrectPick(null);
    setHeldTile(null);
  ...
  useEffect(() => {
    ...
    tickStreak();
    let cancelled = false;
    // Load the vocabulary from the Node backend; fall back to the bundled list.
    fetchWords().then((words) => {
      if (cancelled) return;
      if (words?.length) bankRef.current = words;
      const session = pickSessionWords(bankRef.current);
      setSessionWords(session);
      startFindRound(session[0]);
    });
    return () => {
      cancelled = true;
    };
  ...
  const restart = () => {
    const words = pickSessionWords(bankRef.current);
  ...
  const progressPct = sessionWords.length
    ? Math.round(((wordIndex + (mode === "end" ? 1 : 0)) / sessionWords.length) * 100)
    : 0;
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-3 rounded-2xl bg-card px-4 py-2 shadow-sm">
              <span className="text-sm font-black text-foreground">
                Word {Math.min(wordIndex + 1, sessionWords.length || WORDS_PER_SESSION)} /{" "}
                {sessionWords.length || WORDS_PER_SESSION}
              </span>
        ...
        <div ref={playAreaRef} className="relative flex-1">
          {mode === "find" && !target && (
            <div className="flex h-full flex-1 items-center justify-center">
              <div className="animate-pop flex flex-col items-center gap-3 rounded-3xl bg-card px-8 py-6 shadow-md">
                <div className="h-8 w-8 animate-spin rounded-full border-4 border-primary/30 border-t-primary" />
                <div className="text-sm font-black text-foreground">Loading words…</div>
              </div>
            </div>
          )}
          {mode === "find" && target && (
